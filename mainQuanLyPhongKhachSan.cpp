...
void NhapPhong(stack& s,int soPhong) {
...
void gopTatCaStack(stack sTang[],stack &merge,int tang) {
	for (int i = 0; i < tang; i++) {
        ...
        while (!isEmpty(temp)) {
            ...
            Push(doiStack, mergePhong);
            // mergePhong = Pop(doiStack); // Lời gọi này có thể không cần thiết
...
//		int soPhong;
		...
		Init(s[i]);
		PhongKhachSan phong;
		NhapPhong(s[i],soPhong); // tối đa 2 phòng 1 tầng cho dễ nhập 
		cout<<"\nNhap -1 de dung lai , Nhap tiep hay bam mot so bat ki :";
		cin>>dungLai;
		i++;
		soTang++;
		demSoPhong = soPhong + soPhong;
	}while(dungLai != -1);
	soPhong = demSoPhong;
}
...
int main(){
	...
	while(doiTuong==1){
        ...
        cout<<"14. Xoa tat ca cac phong ! :"<<endl;
        cout<<"15. Nhap so tang muon sua doi ";
        cout << "4. Thoat" << endl;
        cout << "Chon chuc nang: ";
        int choice;
        cin>>choice;
...
//			stack sTang[100];
		...
		if(choice==20){
			...
			cout<<"Dem so phong :"<<soPhong<<endl;
		}
		
		if(choice==2){
			Init(merge);
			gopTatCaStack(sTang,merge,tang);
			cout<<"\nSo phong co la :"<<len(merge)<<endl;
		}
		if(choice==3){
